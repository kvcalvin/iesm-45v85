# *************************************************
# * This script plots maps of the change in total ecosystem due to coupling
# * 
...
for ( v in variables ) { 
  ...
  if ( v == "TOTECOSYSC" ) {
    # Truncate the scale for Total Ecosystem Carbon so it plots better
    Fig.DAT$DIFF[ Fig.DAT$DIFF > 500] <- 500
    Fig.DAT$DIFF[ Fig.DAT$DIFF < -500] <- -500
  } else if ( v == "NPP" ) {
    # Truncate the scale for NPP so it plots better
    Fig.DAT$DIFF[ Fig.DAT$DIFF > 1e-06] <- 1e-06
    Fig.DAT$DIFF[ Fig.DAT$DIFF < -1e-06] <- -1e-06
  }
  ...
  ggsave( width=10, height=5, dpi=500, paste( v, "_COUPLING_MAP_2071to2090.jpg", sep=""))
}